  ...
  <section class="content">
    ...
    <div class="col-md-12">
      <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form role="form" action="{{route('staff.receipt.edit.post', ['receipt_id' => $receipt->id])}}" method="post">
          {{csrf_field()}}
          <div class="card-body">
            <div class="form-group">
              <label>Tên Khách Hàng:</label> {{  $receipt->client->name }}   |   
              <label>Số Điện Thoại:</label> {{ $receipt->client->phone }}    |    
              <label>Ngày Sinh:</label> {{ $receipt->client->birthday }}
            ...
            <div class="form-group">
              ...
              <select name="staff_id" id="staff_id" class="form-control select2" style="width: 100%;">
                ...
                <option value="{{$data->id}}" @if ($data->id == $receipt->staff_id) selected @endif>{{$data->name}}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              ...
              <select name="branch_id" id="branch_id" class="form-control select2" style="width: 100%;">
                ...
                <option value="{{$data->id}}"  @if ($data->id == $receipt->branch_id) selected @endif>{{$data->name}}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              ...
              <select name="field_id" id="field_id" class="form-control select2" style="width: 100%;">
                ...
                <option value="{{$data->id}}"  @if ($data->id == $receipt->field_id) selected @endif>{{$data->name}}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              <label for="note">Ghi chú:</label>
              <input name="note" type="text" class="form-control" id="note" value="{{ $receipt->note }}" placeholder="Nội dung ghi chú">
            </div>
          </div>
          <!-- /.card-body -->
