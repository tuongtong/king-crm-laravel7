...
@section('head')
<title>DEMO20 | Quản lý ca làm việc</title>
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card card-primary">
            <form role="form" action="{{route('staff.shift.manager.post')}}" method="post">
              {{csrf_field()}}
              <div class="card-body">
                <table class="table table-bordered">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        @if(isset($allshift[$s][$day]))
                        @foreach($allshift[$s][$day] as $shift)
                        <div class="form-group">
                          <label>
                            <input type="hidden" name="accept[{{$shift->id}}]" class="minimal" value="0">
                            <input type="checkbox" name="accept[{{$shift->id}}]" class="minimal" value="1" @if($shift->is_accept) checked @endif >
                            {{ $shift->staff->name }}
                          </label>
                        </div>
                        @endforeach
